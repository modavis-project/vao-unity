...
if (codemeta.isBasedOn?.identifier !== `https://doi.org/${standardDoi}` || codemeta.isBasedOn?.url !== standardRepository)
...
for (const [name, expected] of Object.entries(expectedHashes)) {
  ...
  if (actual !== expected) throw new Error(`Published VAO 0.4.0 artifact changed: ${name}`);
}
