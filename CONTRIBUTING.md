...
The GitHub Unity workflow is opt-in because GameCI requires a Unity license. Repository maintainers can enable it by setting the `UNITY_TESTS_ENABLED` repository variable to `true` and configuring either the `UNITY_LICENSE` secret or the `UNITY_SERIAL`, `UNITY_EMAIL`, and `UNITY_PASSWORD` secrets. The repository-quality workflow and all local test scripts remain available without those credentials.
